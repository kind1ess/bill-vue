    ...
    <el-table :data="orderPreviewList" border style="width: 100%" height="93%" stripe>
      ...
      <el-table-column label="状态">
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.statusId===5||scope.row.statusId===10" type="success">{{scope.row.statusName}}</el-tag>
...
export default {
  ...
  methods: {
    ...
    showTable(statusId,page,size){
      orderApi.findOrderPreviewVoByStatusIdPageBy(statusId,page,size).then(response => {
        this.orderPreviewList = response.data.data.orderPreviewDtoList;
      })
    },
    getTotal(){
      orderApi.getCountBy(this.statusId).then(response => {
